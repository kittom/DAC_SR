...
# PySR Activation and Execution Script (Rounding)
# This script activates the pysr_env conda environment and runs the PySR symbolic regression with rounding support
...
cd "$SCRIPT_DIR/../../SR_algorithms_rounding/PySR"
...
echo "Running PySR symbolic regression (with rounding) on CSV data..."
python run_pysr.py "$CSV_FILE"

if [ $? -eq 0 ]; then
    echo "PySR (rounding) execution completed successfully!"
    ...
    echo "Error: PySR (rounding) execution failed!"
    exit 1
fi
...
echo "PySR (rounding) execution completed!" 
...
# TPSR Activation and Execution Script (Rounding)
# This script activates the tpsr conda environment and runs TPSR symbolic regression (rounding)
...
cd "$SCRIPT_DIR/../../SR_algorithms_rounding/TPSR"
...
echo "Running TPSR symbolic regression (rounding) on CSV data..."
python run_tpsr_on_csv.py "$CSV_FILE" --noise "$NOISE"

if [ $? -eq 0 ]; then
    echo "TPSR (rounding) execution completed successfully!"
    ...
    echo "Error: TPSR (rounding) execution failed!"
    exit 1
fi
...
echo "TPSR (rounding) execution completed!" 
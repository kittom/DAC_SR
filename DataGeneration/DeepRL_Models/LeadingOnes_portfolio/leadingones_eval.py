...
class LeadingOnesEval():
    ...
    def eval(self, n_steps, with_skip=False) -> bool:
        ...
        for inst_id in self.inst_ids:
            ...
            # Clip policy to ensure we don't try to flip more bits than available
            policy = []
            for i, v in enumerate(policy_unclipped):
                max_bits_available = n - i  # At fitness level i, we have n-i bits available
                clipped_value = np.clip(v, 1, max_bits_available)
                policy.append(clipped_value)
            ...
            if self.verbose>=1:
                print(f"\t[env: {self.name}] instance: {inst_id}. Runtime: {runtime_mean} +/- {runtime_std}")
                # Debug: show some policy actions
                if inst_id == 0:  # Only for first instance to avoid spam
                    print(f"\t\tPolicy actions (first 10): {policy[:10]}")
                    print(f"\t\tPolicy actions (last 10): {policy[-10:]}")
            if self.verbose>=2:
                print("\t" + " ".join([str(v) for v in policy]))

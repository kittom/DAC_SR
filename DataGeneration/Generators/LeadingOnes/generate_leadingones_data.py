#!/usr/bin/env python3
"""
Consolidated LeadingOnes data generation script.
...
def generate_leading_ones_data(instance_sizes=None, data_type='continuous', output_dir=None, 
    ...
    for instance_size in instance_sizes:
        for current_state in range(instance_size + 1):
            # LeadingOnes uses instance_size / (current_state + 1)
    ...
    if output_dir is None:
        if hidden_variables:
            out_dir = os.path.join(PROJECT_ROOT, f'DataSets/Ground_Truth_Hidden/LeadingOnes/{data_type}')
        elif noise_level > 0.0:
            out_dir = os.path.join(PROJECT_ROOT, f'DataSets/Ground_Truth_Noise/LeadingOnes/{data_type}')
        else:
    ...
    if hidden_variables:
        ...
        ground_truth_equation = "500/(x1 + 1)" if data_type == 'continuous' else "round(500/(x1 + 1))"
    else:
...
def main():
    ...
    print(f"Generating LeadingOnes data with parameters:")

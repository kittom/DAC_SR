...
OUTPUT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../DataSets/Ground_Truth_Hidden/PSACMAES'))

# Only these columns (alpha and beta are hidden)
CSV_COLS = ['lambda', 'ptnorm', 'gamma_theta', 'next_lambda_unrounded']

def run_psa_cmaes_benchmark_hidden(benchmark_name, fid, iterations, output_dir, data_type='continuous', noise_level=0.0, noise_type='gaussian'):
    dim = 10
    budget_factor = 2500
    algo = 'psa'
    function_id = fid
    ...
    try:
        ...
        with open(csv_path, 'w', newline='') as csvfile:
            ...
            psa_beta = cma.parameters.psa_beta  # Hidden from dataset
            ptnorm = cma.parameters.ptnorm
            alpha = cma.parameters.alpha  # Hidden from dataset
            ...
            # Only write lambda, ptnorm, gamma_theta, next_lambda_unrounded (alpha and beta are hidden)
            writer.writerow([
                lambda_, ptnorm, gamma_theta, next_lambda_unrounded
            ])
...
def main():
    parser = argparse.ArgumentParser(description="Generate PSA-CMA-ES ground truth data for multiple benchmarks with hidden alpha and beta values.")
    ...
    parser.add_argument('--noise-level', type=float, default=0.0, help='Noise level (std for gaussian, range for uniform)')
    ...
    for benchmark_name, fid in BENCHMARKS:
        ...
        csv_path = run_psa_cmaes_benchmark_hidden(benchmark_name, fid, iterations, output_dir, data_type, noise_level, noise_type)

...
def generate_one_max_ground_truth_hidden(instance_size=500, noise_level=0.0, noise_type='gaussian'):
    """Generate OneMax data with instance size hidden from the dataset."""
    current_state_data = []
    bitflip_data = []
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Generate OneMax ground truth data with hidden instance size')
    ...
    print(f"Generating OneMax data for instance size: {args.instance_size} (hidden from dataset)")
    print(f"Noise type: {args.noise_type}, Noise level: {args.noise_level}")
    
    if args.output_dir is not None:
        base_out_dir = args.output_dir
    else:
        base_out_dir = os.path.join(PROJECT_ROOT, 'DataSets/Ground_Truth_Hidden/OneMax')
    
    # Continuous
    cont_dir = os.path.join(base_out_dir, 'continuous')
    os.makedirs(cont_dir, exist_ok=True)
    df_cont = generate_one_max_ground_truth_hidden(args.instance_size, args.noise_level, args.noise_type)
    cont_path = os.path.join(cont_dir, 'GTOneMax.csv')
    df_cont.to_csv(cont_path, index=False, header=False)
    write_results_files(cont_dir, 'sqrt(500/(500-x1))')
    ...
    disc_path = os.path.join(disc_dir, 'GTOneMax.csv')
    df_disc.to_csv(disc_path, index=False, header=False)
    write_results_rounding_file(disc_dir, 'round(sqrt(500/(500-x1)))')

...
def generate_one_max_ground_truth_with_noise(instance_sizes=None, noise_level=0.1, noise_type='gaussian'):
...
if __name__ == "__main__":
    ...
    if args.output_dir is not None:
        ...
        base_out_dir = os.path.join(PROJECT_ROOT, 'DataSets/Ground_Truth_Noise/OneMax')
    # Continuous
    cont_dir = os.path.join(base_out_dir, 'continuous')
    os.makedirs(cont_dir, exist_ok=True)
    df_cont = generate_one_max_ground_truth_with_noise(instance_sizes, args.noise_level, args.noise_type)
    cont_path = os.path.join(cont_dir, 'GTOneMax.csv')
    df_cont.to_csv(cont_path, index=False, header=False)
    write_results_files(cont_dir, 'sqrt(x1/(x1-x2))')
    # Discrete (rounded)
    ...
    write_results_rounding_file(disc_dir, 'round(sqrt(x1/(x1-x2)))')

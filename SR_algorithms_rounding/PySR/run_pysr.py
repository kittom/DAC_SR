...
def main():
    ...
    print("Running PySR symbolic regression with rounding support...")
    try:
        model = PySRRegressor(
            niterations=40,
            binary_operators=["+", "-", "*", "/"],
            unary_operators=["sin", "cos", "exp", "log", "round"],
            extra_sympy_mappings={"round": lambda x: x.round()},
            extra_torch_mappings={"round": lambda x: x.round()},
            progress=True,
        ...
        error_message = f"Error running PySR: {e}"
        print(error_message)
    ...
    if equation is None:
        equation = error_message or "ERROR: PySR failed with unknown error"
    # Update or create results_rounding.csv in the same directory as the input CSV
    ...
    try:
        ...
        # Do not exit here; just print the error
